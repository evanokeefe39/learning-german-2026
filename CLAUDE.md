...
- **Shared components**: `src/components/{filter-bar,score-display,finished-screen,empty-state,practice-banner,navbar}.tsx` — reusable UI extracted from test components
...
- **Storage**: `src/lib/storage.ts` — localStorage helpers for wrong words, mastery tracking, and attempt leaderboard
...
- Mastery tracking stores per-word correct answer counts (`mastery:{mode}`) — changing the "known words" threshold dynamically recalculates without losing data
...
- Settings panel uses `createPortal` to render outside the header, avoiding stacking context issues
- UI uses "known words" terminology (not "mastered") for the word familiarity feature
